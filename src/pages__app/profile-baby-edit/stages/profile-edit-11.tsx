...
const ProfileEditElevenStage = () => {
  ...
  const { setAbout, about } = useAnketsBabysitter();
    ...
    const onSubmit = async (data: any) => {
      ...
      const text = data.about.getCurrentContent().getPlainText().trim();
      if (!text) {
        setErrors('Поле обязательно для заполнения')
        return;
      }
    ...
    <div className={styles['profile-edit-stage']}>
      ...
      <div className='flex gap-[24px] w-full  max-[768px]:flex-col'>
        <div className='w-full mt-[25px]'>
            <span className='font-normal text-[#7C8092] cursor-pointer text-[16px] underline leading-[26px] block mb-[16px]'>
